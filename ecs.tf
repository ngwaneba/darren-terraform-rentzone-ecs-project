...
resource "aws_ecs_task_definition" "ecs_task_definition" {
  ...
  network_mode              = "awsvpc"
  requires_compatibilities  = ["FARGET"]
  cpu                       = 256
  memory                    = 512
  ...
  container_definitions     = jsonencode([
    {
      ...
      environmentFiles = [
        {
          value = "arn:aws:s3:::aws_s3_bucket.env_file_bucket/${var.nv_file_name}"
          type  = 
        }
      ]
...
resource "aws_ecs_service" "ecs_service" {
  ...
  network_configuration {
    subnets                 = [aws_subnet.private_app_subnet_az1.id, aws_subnet.private_app_subnet_az1.id]
